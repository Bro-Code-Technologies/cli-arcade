...
def sync_scores(scores_map):
    """
    Bulk-sync scores for multiple games in a single API call.

    Parameters
    ----------
    scores_map : dict
        Mapping of game slug -> score dict,
        e.g. ``{'byte_bouncer': {'score': {'player': 'Alice', 'value': 100}}}``.

    Returns
    -------
    dict or None
        The merged state for all submitted games, or None on failure.
    """
    if not is_enabled():
        return None
    base = _base_url()
    url = f'{base}/api/apps/cli-arcade/scores/sync'
    try:
        session = _get_session()
        resp = session.post(url, json=scores_map, timeout=_TIMEOUT)
        if resp.status_code == 200:
            return resp.json()
        return None
    except Exception as e:
        warnings.warn(f'CLI Arcade API sync_scores failed: {e}')
        return None


def delete_scores(game):
    """
    Delete all scores for a game on the server.
    ...
    -------
    bool
        True on success (204 No Content), False on failure or when disabled.
    """
    if not is_enabled():
        return False
    base = _base_url()
    ...
    try:
        ...
        warnings.warn(f'CLI Arcade API delete_scores failed: {e}')
        return False
